...
clone_dotfiles() {
  ...
  if ! confirm "Do you want to skip the .local/bin custom scripts installing process?" 0; then
    ...
    mkdir -p $HOME/.local/bin
		...
		local__bin_scripts="monitor-config wl-random-wallpaper volume percentage brightness checkupdates audio-toggler wl-wallpapers-rotator wl-wallpapers-previewer spanish-kb-copier random"
...
create_local__bin_symlinks() {
  ...
  to_paste="monitor-config wl-random-wallpaper wl-wallpapers-rotator volume percentage brightness checkupdates audio-toggler wl-wallpapers-previewer spanish-kb-copier random"
...
setup_wallpaper() {
  ...
  if confirm "Do you want to setup a default wl wallpaper (05.png)? " 1; then
    wl_cmd set 05.png
  fi
